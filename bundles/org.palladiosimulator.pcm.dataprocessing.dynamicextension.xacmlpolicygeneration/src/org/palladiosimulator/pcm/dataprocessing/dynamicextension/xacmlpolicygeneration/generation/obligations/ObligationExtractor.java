...
/**
 * Extracts the obligations from the context information of a related
 * characteristics.
 * 
...
public class ObligationExtractor extends Extractor<ObligationExpressionsType> {
	...
	protected ObligationExpressionsType extractOneElement(final int index) {
		final List<Context> allContexts = ContextHandler.getContexts(getRelatedCharacteristic(), index);
		// getting all obligations contained in the context information
		final  List<ObligationExpressionType> obligationsList 
			= ObligationRegistry.getInstance().getAll(allContexts).stream()
				.map(Obligation::getObligations)
				.flatMap(List::stream)
